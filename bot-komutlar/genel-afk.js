...
exports.run = async (client, message, args) => {
  ...
  if (args[0]) {
    ...
    if (sebep.includes("@everyone")) return message.channel.send(`${message.author} Akıllısın. Ama ben senden akıllıyım :) Maalesef bu komutla everyone çekemezsin.`).then(a => a.delete({timeout: 4500}));
    if (sebep.includes("@here")) return message.channel.send(`${message.author} Akıllısın. Ama ben senden akıllıyım :) Maalesef bu komutla here çekemezsin.`).then(a => a.delete({timeout: 4500}));
